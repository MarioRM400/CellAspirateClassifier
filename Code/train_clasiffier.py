...
def main():
    ...
    train_transform_img = tvt.Compose([
        ...
        # tvt.ColorJitter(brightness=(0.8, 1.2), contrast=(0.8, 1.2), saturation=0., hue=0.),
        # tvt.Grayscale(num_output_channels=3),
        # tvt.RandomAffine(degrees=(-ANGLE, ANGLE),
        #                  shear=(-SHEAR_FACTOR, SHEAR_FACTOR),
    ...
    try:
        ...
        model.load_state_dict(torch.load('weights/oocyte_maturity_clf-pre.pt'))
        # model.load_state_dict(torch.load('weights\oocyte_maturity_clf-pre.pt', map_location=torch.device('cpu')))
